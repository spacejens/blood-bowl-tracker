...
describe('AppModule', () => {
  it('wires every dev-workflow subcommand service with its dependencies resolved', async () => {
    ...
    expect(moduleRef.get(ReviewLockStateService)).toBeInstanceOf(
      ReviewLockStateService,
    );
    expect(moduleRef.get(ReviewLockService)).toBeInstanceOf(ReviewLockService);
    expect(moduleRef.get(ReviewLockArgsService)).toBeInstanceOf(

...
@Injectable()
export class ReviewMatchConfigService {
  private readonly config: Record<string, unknown>;

  constructor(
    @Inject(REVIEW_MATCH_CONFIG_PATH) private readonly filePath: string,
  ) {
  ...
  getDatabaseUrl(): string {
    ...
    if (typeof url !== 'string' || url === '') {
      throw new Error(
        'database.url is not set in review-match-config.json5. Set it to the ' +
  ...
  getMatchesPerStratum(): number {
    const raw = this.config.matchesPerStratum;
    if (raw === undefined) {
      return DEFAULT_MATCHES_PER_STRATUM;
    }
    if (typeof raw !== 'number' || !Number.isInteger(raw) || raw < 1) {
      throw new Error(
        'matchesPerStratum in review-match-config.json5 must be a positive ' +
  ...
  getDataDir(source: ReviewSource): string {
    ...
    if (typeof dir !== 'string' || dir === '') {
      throw new Error(
        `${source}.dataDir is not set in review-match-config.json5. Set it to ` +
  ...
  /** Source-specific external match ids to always include in the report. */
  getOverrides(source: ReviewSource): string[] {
    ...
    if (!Array.isArray(raw)) {
      throw new Error(
        `overrides.${source} in review-match-config.json5 must be an array of ` +
          'external match ids.',
      );
    }

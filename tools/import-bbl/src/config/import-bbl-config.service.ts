 ...
 */
export const PRODUCTION_IMPORT_BBL_CONFIG_PATH = resolve(
  process.cwd(),
  'import-bbl-config.production.json5',
);
 ...
 */
export function resolveImportBblConfigPath(): string {
...
export class ImportBblConfigService {
  ...
  getApiToken(): string {
    ...
    if (token === undefined) {
      throw new Error(
        'connection.apiToken is not set in import-bbl-config.json5. Set it ' +
          'to the bearer token this tool authenticates with; it must match ' +
          'the API_TOKEN_IMPORT_BBL value in apps/discord-bot/.env.',
      );
    }
    return token;
  }

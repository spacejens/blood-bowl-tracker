 ...
 * one mercenary ("Big Guy") position's characteristics under one rules set.
 * TP supplies no characteristics for a mercenary hire anywhere -- not in a
 * roster catalog, not on the match-embedded hire itself -- so this is
 * curated, config-supplied data; see `MercenaryConfigService`. Messages carry
 * only the tail -- `ConfigErrorMessageService` prepends the `MERCENARIES[i]`
 * location.
 */
export const mercenaryEntrySchema = z.object(
  {
...
/**
 * The `mercenaries` array as `MercenaryConfigService` reads it: a (possibly
 * empty) array, whose elements are parsed one at a time by
 * `mercenaryEntrySchema` so each error can name `MERCENARIES[i]` rather than
 * a path through this array. Unlike `league.eras`, an empty or entirely
 * absent setting is valid -- it just means no mercenary hire has been curated
 * yet, which the config-reading service turns into an empty lookup table
 * rather than an error.
 */
export const mercenaryShellSchema = z.array(z.unknown(), {
  error: NOT_AN_ARRAY,
});

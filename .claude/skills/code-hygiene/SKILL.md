...
7. **Build the Docker image**, but only once step 6 passes — `pnpm verify` only confirms nothing else in the repo broke under the new Node; it does not confirm the new base image actually builds, because `verify` reinstalls and rebuilds under the *host's* Node/OS, not inside the Docker build context, and an alpine base bump can break native-module builds in ways `pnpm verify` alone won't catch:
   ...
   ```
8. **If both pass:** one commit covering `.nvmrc`, the Dockerfile, every changed `package.json`, and `pnpm-lock.yaml` together — e.g. "Update Node to 28 (LTS), sync .nvmrc and @types/node". This is one logical change; splitting it across commits would leave intermediate commits with a broken invariant (Dockerfile and `.nvmrc` disagreeing, `@types/node`'s major mismatched, or the lockfile out of sync with the manifests it was generated from — which would break CI's `--frozen-lockfile` install). Committing only after both checks pass avoids ever committing a Node bump whose Docker image doesn't build.

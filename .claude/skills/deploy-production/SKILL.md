...
3. Run the query through `tools/ai-helpers`'s `run-production-query` subcommand (build it first with `pnpm --filter @blood-bowl-tracker/ai-helpers run build` if `dist/main.js` is missing), feeding the query text via stdin in the same stdin-heredoc form used elsewhere in this repo's skills for `write-file` and `post-review-questions`:
   ```bash

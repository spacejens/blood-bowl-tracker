...
2. **Worktree removal.** How to remove it depends on *how* this session entered it — `EnterWorktree` has two modes, and only one of them can be undone by `ExitWorktree(action: "remove")`:
   ...
   - **No `EnterWorktree` involved at all** (worktree isolation was declined) — offer a plain `git worktree remove <path>`; no session-relocation step is needed.

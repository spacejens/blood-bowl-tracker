...
- **Phase 3 reuses `write-issue`'s Phase 3 (Draft and create) by reference**, in its new-parent-with-sub-issues mode — read `.claude/skills/write-issue/SKILL.md` and follow that phase, rather than duplicating its steps here, so the two skills can't drift out of sync. This is the same cross-reference pattern `code-hygiene` uses for `develop-feature`'s Setup phase.
...
- **Kind labels come from the "Issue labels" section of [docs/development-workflow.md](../../../docs/development-workflow.md)**, reached through `write-issue`'s Phase 3 step 4, which already applies it. Do not duplicate that logic in this file either.

## Phase 1: Review — fan out one subagent per criterion

Dispatch one read-only `Agent` per criterion in the fixed list below, each scanning the **whole** repository for that one criterion. Every criterion is independent, so dispatch them **in parallel — a single message with one `Agent` tool call per criterion in the list** — not one at a time.
...
- Explicitly request a stronger model for these judgment-heavy dispatches — pass `model: "opus"` on each `Agent` call, along with `effort: "high"` if the `Agent` tool available in the session accepts an effort parameter — rather than inheriting from the invoking session, since this skill is a rare, thorough pass where the extra reasoning quality is worth the cost.
...
- Each dispatch prompt contains, in this order: the repo root and the `cd` prefix rule; the criterion's own instructions from the fixed list below, quoted in full; a pointer to the specific `CLAUDE.md` section (or `docs/` file) that defines the convention being audited, so the agent reads the authoritative wording rather than working from the summary — for a criterion anchored to precedent rather than a written rule, point at the concrete existing code location that exemplifies the convention instead, so the agent compares against real precedent rather than an abstract standard; the "what does not count" exclusions for that criterion; the read-full-files instruction above; and the finding report format below.
...
7. **Discord bot output inconsistencies.** Find commands/embeds in `apps/discord-bot` that present the same kind of data differently from how other commands present equivalent data — inconsistent formatting (e.g. one command bolding a stat another leaves plain), inconsistent terminology for the same concept, or inconsistent display conventions for equivalent values. Anchor: compare against how the same kind of data is already presented elsewhere in `apps/discord-bot`, not a fixed spec. Does not count: a deliberate difference already documented as intentional (e.g. the bot's existing zero-vs-null dash display convention), or commands that legitimately show different kinds of data.

8. **Vague naming.** Find classes, functions, and variables named too generically to convey their actual purpose or contents — e.g. `data`, `helper`, `process()`, `Manager`, `info` used where a more specific name is readily available and would meaningfully aid a reader. Does not count: names that are appropriately generic for a truly generic utility (a type parameter `T`, a genuinely generic `options` parameter), or a name already covered by an ESLint rule.

9. **Error handling/logging inconsistency.** Find services that handle equivalent failure conditions differently from how comparable services elsewhere in the same package/app handle them — inconsistent error types (a plain `Error` where sibling services throw a specific NestJS exception), inconsistent log levels for equivalent severities, or inconsistent log message shapes. Anchor: compare against how error handling is already done elsewhere in the same package/app. Does not count: a genuinely different severity or failure mode that warrants different handling.

10. **Duplicate domain types/DTOs.** Find the same domain concept (e.g. a player stat shape, a match summary) redefined slightly differently across packages instead of sharing one type already defined in `packages/api-contract`. Anchor: `packages/api-contract`'s existing schemas as the source of truth for shared domain types. Does not count: a type genuinely internal to one package/service that never crosses a package boundary, or two types that only coincidentally share field names but represent different concepts.

11. **Null/undefined convention drift.** Find inconsistent choices for representing "no value" across similar fields or services — one service returning `null` for a missing entity where a comparable service returns `undefined`, or a DTO mixing both conventions for equivalent optional fields. Anchor: compare against how comparable fields/services elsewhere in the same package already represent absence. Does not count: a difference required by an external API or library's own contract (e.g. drizzle returning `undefined` where a DTO field is explicitly optional).

12. **Env var/config naming inconsistency.** Find inconsistent naming or prefixing conventions for environment variables and config keys across `apps/` and `tools/` — e.g. one tool prefixing its env vars with its own name and another not, or inconsistent casing/separator conventions for equivalent kinds of settings. Anchor: compare against how existing apps/tools already name their env vars and config keys. Does not count: an env var whose name is dictated by an external system (e.g. a required cloud-provider variable name).

13. **Date/time handling inconsistency.** Find inconsistent representation of dates and times for equivalent data — one service storing/passing a `Date` object where a comparable service uses an ISO string, or inconsistent timezone assumptions for game/competition dates across services. Anchor: compare against how comparable date/time fields elsewhere in the same package already are represented. Does not count: a difference driven by an external API's own format (e.g. a source system returning dates as strings).

14. **Domain constant duplication.** Find domain values — race names, position codes, rules-set identifiers, or similar — hard-coded or redefined in a package other than `packages/domain-enums` where a canonical definition already exists there, instead of importing the shared value. Anchor: `packages/domain-enums`'s existing exports as the source of truth. Does not count: a value genuinely specific to one tool/app's own internal logic that does not represent a domain concept `packages/domain-enums` already models.
...
- Its step 3 ("Create it" / "Revise the draft") runs once per confirmed candidate, so a pass with many confirmed children means confirming the same list twice — once in Phase 2's multi-select, once more per drafted issue. That repetition is inherited from `write-issue` as-is: each draft is new information (the actual title and body) the multi-select didn't show, so it is not a step to skip or batch.
- Its step 4 (kind labels) applies the "Issue labels" section of [docs/development-workflow.md](../../../docs/development-workflow.md). Most candidates are convention or documentation drift under `tools/`, tests, CI, or Claude skills, so `development` is the expected label for most of them; a candidate found inside `apps/`, or inside a `packages/` package whose specific finding affects the deployed application, is `internal` instead — a finding in a shared `packages/` package (e.g. `packages/db`, `packages/api-client`) that only affects a `tools/` CLI's own use of it stays `development`, per that section's rule for packages that back both. Judge each candidate on its own merits against that section rather than applying either label by default.
- Its step 5 creates each child with `--parent <parent issue number>`.

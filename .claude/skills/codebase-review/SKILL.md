...
- Its step 4 (kind labels) applies the "Issue labels" section of [docs/development-workflow.md](../../../docs/development-workflow.md). Most candidates are convention or documentation drift under `tools/`, tests, CI, or Claude skills, so `development` is the expected label for most of them; a candidate found inside `apps/` or the `packages/` that back them is `internal` instead — judge each candidate on its own merits against that section rather than applying either label by default.

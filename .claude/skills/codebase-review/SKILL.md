...
- Its step 4 (kind labels) applies the "Issue labels" section of [docs/development-workflow.md](../../../docs/development-workflow.md). This is process/tooling work with no end-user-visible effect, so `development` is the expected label for most or all candidates — but judge each candidate on its own merits against that section rather than applying `development` by default.

...
Both states share the same two preconditions: the comment's author login matches `coderabbit` (case-insensitive) **and** its `body` contains a `<!-- recent_review_start -->...<!-- recent_review_end -->` section. They differ only in what that section's contents match:

- **Completion state** — the contents match a "no actionable comments" style phrase (tolerant wording, case-insensitive). This is the same rule `WaitForPrReviewService`'s `completionFilter` applies in `tools/ai-helpers/src/wait-for-pr-review/wait-for-pr-review.service.ts`, deliberately worded identically in both places so the two can be kept in sync by inspection.
- **In-progress state** — the contents match a "currently processing new changes" style phrase (tolerant wording, case-insensitive). Match on that core phrase rather than CodeRabbit's full sentence: its punctuation and trailing text drift between revisions of the comment ("...This may take a few minutes, please wait..." in some, absent in others), and the core phrase is the stable part.

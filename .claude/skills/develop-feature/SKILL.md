...
5. **Check whether the branch is too large for one PR.** CodeRabbit refuses to review a PR past a file-count limit, posting a "Review skipped — Too many files!" comment instead of a review. Phase 6's review loop cannot tell that apart from a slow review, so it would wait out its full iteration budget for a review that is never coming. Check here instead — the branch is now in its final, all-findings-resolved state, so what is measured is exactly what would otherwise become one PR.
   ...
   **Accepted residual risk:** this check runs before Phase 6 step 1's `git merge origin/main`, so it cannot see files `main` has changed since this branch started. If `main` drifts by more than `CODERABBIT_SAFE_FILE_LIMIT`'s safety margin before Phase 6's merge runs, the PR's real file count could still exceed the limit despite this check passing. This is a deliberate trade-off — checking after the `main` sync would require restructuring Phase 5/6 for a case the safety margin already absorbs in the common case — not an oversight.
   ...
   - `"splitNeeded": true` with an empty `parts` array and an `unsplittable` object — a single task's own commit touches more files than the limit, so no valid split exists. **Pause** — report the offending `taskNumber`, `label`, and `fileCount`, and ask the developer via `AskUserQuestion`, offering two genuine options: **Open one oversized PR anyway** (record this as the accepted-oversized decision and proceed into Phase 6, which runs as written for the single-PR case except that its review loop is skipped — see Phase 6's own intro and step 5 — accepting that CodeRabbit will skip the review) and **Stop here** (halt the skill, leaving the branch unpushed for the developer to split by hand). Per this project's `AskUserQuestion` convention (`CLAUDE.md`), do not add an explicit free-text or chat option — both are provided automatically.
   - `"splitNeeded": true` with an empty `parts` array and an `unsplittableWholeBranch` object — the last part, once remeasured up to the branch tip, is still over the limit and there is no further checkpoint to cut it at, so no valid split exists either. This covers two cases: packing could only ever produce one usable part (every section folded back together, or there was only one to begin with), or packing produced two or more parts that all fit fine except the last one, which self-review fix commits made after the final task's commit pushed back over the limit — in that second case the earlier, already-valid parts are discarded too, since a partial split is not worth the added complexity here. No single task is to blame, so there is no `taskNumber`/`label` to report — just `fileCount`, which in the second case is the size of the unsplittable last part specifically, **not** the whole branch's `totalFileCount` (the two can differ; report whichever the JSON actually carries under `unsplittableWholeBranch.fileCount`, not a value recomputed or assumed from `totalFileCount`). **Pause** — report `unsplittableWholeBranch.fileCount` and the limit, and ask the developer via `AskUserQuestion`, offering the same two genuine options as the `unsplittable` case above: **Open one oversized PR anyway** and **Stop here**. Same `AskUserQuestion` convention applies — no explicit free-text or chat option.
...
3. **Acquire the review lock, then create the PR.** Creating the PR is itself what triggers CodeRabbit's first review, and CodeRabbit's review rate limit is shared across every PR in the repo. Several `develop-feature` sessions running in parallel worktrees on this machine would otherwise all consume that quota at once and all crawl. Take the machine-wide review lock here and hold it through the end of step 5's review loop, so exactly one session drives a review loop at a time and the rest wait their turn in the order they started waiting. See `docs/development-workflow.md`'s "Serializing review activity across parallel sessions".
   ...
   **Release the review lock before asking, and re-acquire if a retry follows** — exactly the same pattern used for the review loop's own timeout/rate-limit/comment-update-failure Pauses below, and for the same reason: the lock was just acquired above, immediately before this very `gh pr create` call, so an unanswered Retry-or-Stop prompt is exactly the kind of unbounded human wait that must never hold it. Run `release-review-lock <holder-id>` immediately before presenting the `AskUserQuestion`, retrying once if it doesn't confirm `{"released": true}` before proceeding anyway. If the developer chooses **Retry**, re-acquire with `acquire-review-lock <holder-id>` (backgrounded the same way as its first use above) before re-running `gh pr create`; if it fails again, repeat this whole release/ask/re-acquire handling. If they choose **Stop**, leave the lock released — the skill is halting.
...
5. **Automated review loop.** An automated review bot reviews every PR in this repo (see `docs/development-workflow.md`). Wait for its review and drive it to completion here rather than leaving it for the developer to notice later. Repeat the wait → handle cycle below for at most **10 iterations total**.
   ...
   **Skip this whole step if Phase 5 step 5 recorded an accepted-oversized decision** (the developer chose **Open one oversized PR anyway** for an `unsplittable` or `unsplittableWholeBranch` outcome). The detection check already confirmed this branch's file count is past the limit CodeRabbit refuses to review at — waiting here would just burn the full 10-iteration budget on a review that cannot arrive, an outcome this whole feature exists to avoid causing. Release the review lock taken in step 3 (`release-review-lock <holder-id>`, retrying once if it doesn't confirm `{"released": true}` before proceeding anyway) — this PR never actually triggers a review, so there is nothing left worth holding it for. Print a brief status line here — in place of the one this step's own ending would otherwise print below — stating that the automated review loop was skipped because the developer accepted an oversized PR, and that this PR needs a closer human read than usual since CodeRabbit will not review it. Then go straight to step 6.

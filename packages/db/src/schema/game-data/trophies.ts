...
const trophiesTable = historyTrackedTable({
  ...
  columns: {
    ...
    // carries a classification, and deliberately without a default: an
    // uncurated trophy must fail loudly rather than silently classify as
    // `direct_source` and never be computed. (The migration that introduces
    // the column gives it a temporary `direct_source` default purely so the
    // ADD COLUMN is safe against a populated table; a follow-up migration
    // drops that default again.)
    ...
    //
    // Like `awardRuleKind`, it carries no default here: the migration that
    // introduces the column gives it a temporary placeholder default so
    // pre-existing rows satisfy `trophies_award_rule` (whose `direct_source`
    // branch requires a non-null procedure) the moment the check is added,
    // and a follow-up migration drops that default again.
    awardProcedure: varchar('award_procedure', { length: 1024 }),

...
describe('CoachesService.resolveBatch', () => {
  it('answers each pair with the coach id that declares it', async () => {
    const { service } = await makeService([
      { ownerId: 4, externalSystemId: 1, externalId: 'bob' },
    ]);

    await expect(
      service.resolveBatch([
        { externalSystemId: 1, externalId: 'bob' },
...
describe('CoachesService.resolve', () => {
  it('answers a single pair with the coach id that declares it', async () => {
    const { service } = await makeService([
      { ownerId: 4, externalSystemId: 1, externalId: 'bob' },
    ]);

    await expect(
      service.resolve({ externalSystemId: 1, externalId: 'bob' }),

  ...
  > {
    const rows = await this.db
      .select({
        ...
        // `players` itself. A stored 0 is passed through as-is — a stale
        // legacy value for the four NOT NULL columns (0 is not a legal value
        // for any of them under any rules set, so there is nothing a
        // database-level fallback could legitimately write), a real value
        // for a `plus_zero_legal` Passing value — because rendering it is

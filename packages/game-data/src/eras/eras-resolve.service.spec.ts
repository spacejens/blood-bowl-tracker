...
async function makeService(rows: unknown[]) {
  ...
  return { service: moduleRef.get(ErasService), chains };
}

describe('ErasService.resolveBatch', () => {
  it('answers each pair with the era id that declares it', async () => {
    const { service } = await makeService([
      { ownerId: 4, externalSystemId: 1, externalId: 'Season 12' },
    ]);

    await expect(
      service.resolveBatch([
        { externalSystemId: 1, externalId: 'Season 12' },
...
describe('ErasService.resolve', () => {
  it('answers a single pair with the era id that declares it', async () => {
    const { service } = await makeService([
      { ownerId: 4, externalSystemId: 1, externalId: 'Season 12' },
    ]);

    await expect(
      service.resolve({ externalSystemId: 1, externalId: 'Season 12' }),

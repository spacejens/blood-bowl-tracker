...
async function makeService(rows: unknown[]) {
  ...
  return { service: moduleRef.get(LeaguesService), chains };
}

describe('LeaguesService.resolveBatch', () => {
  it('answers each pair with the league id that declares it', async () => {
    const { service } = await makeService([
      { ownerId: 4, externalSystemId: 1, externalId: 'premier' },
    ]);

    await expect(
      service.resolveBatch([
        { externalSystemId: 1, externalId: 'premier' },
...
describe('LeaguesService.resolve', () => {
  it('answers a single pair with the league id that declares it', async () => {
    const { service } = await makeService([
      { ownerId: 4, externalSystemId: 1, externalId: 'premier' },
    ]);

    await expect(
      service.resolve({ externalSystemId: 1, externalId: 'premier' }),

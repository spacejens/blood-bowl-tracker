...
- **`tools/ai-helpers`** — NestJS CLI application providing worktree-aware helpers the Claude Code skills in `.claude/skills/` call instead of hand-rolling the equivalent shell: `resolve-main-root` (locate the main checkout from a worktree), `check-main-stray` (uncommitted files and unpushed commits left in the main checkout), `sync-gitignored` (copy/symlink missing gitignored dev config into a worktree), `check-drift` (gitignored config that differs between a worktree and the main checkout), and `write-file` (write a file at a repo-relative path, reading its content from stdin — used to save specs and plans through the `docs/plans` symlink, which the Claude Code Write tool refuses to write through), and `wait-for-pr-review` (poll `gh` internally for a submitted PR review until one appears or a timeout elapses, printing one JSON result — a single command a worktree-isolated session can run, unlike the inline multi-line poll loop it replaces). Run as `node tools/ai-helpers/dist/main.js <subcommand>` from the repo root after `pnpm build`; every subcommand prints JSON on stdout

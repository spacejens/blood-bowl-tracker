...
- **`packages/import-tp-live`** — NestJS module that imports one TP team, and its players, from TP's live API: fetches the roster via `packages/scrape-tp`, parses it via `packages/parse-tp`, resolves its era, and upserts it through the same team/player import services `tools/import-tp`'s bulk import runs on; builds its roster requests from `packages/tp-paths` (see [docs/import-tp-live/index.md](docs/import-tp-live/index.md))
...
- **`packages/tp-paths`** — NestJS module with TP's URL paths as TP's own frontend requests them (currently a roster's API path and its page path); pure path building with no I/O, so both `tools/download-tp` and `packages/import-tp-live` depend on it

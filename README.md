...
- **`packages/import-tp-live`** — server-side NestJS module that imports one TP roster's team and players straight through `packages/game-data`: the live import (fetch via `packages/scrape-tp`, parse via `packages/parse-tp`, era auto-resolution) and the implementation behind the `tpRosters.import` procedure `tools/import-tp` calls (see [docs/import-tp-live/index.md](docs/import-tp-live/index.md))

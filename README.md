...
- **`tools/review-player`** — NestJS CLI application that renders a side-by-side HTML report of everything known about a sampled set of players — identity, team, position and era, plus their star player points computed from match events versus the totals stored on `game_data.players` (see [docs/review-player/index.md](docs/review-player/index.md))
...
- **`tools/ai-helpers`** — NestJS CLI application providing helpers the Claude Code skills in `.claude/skills/` call instead of hand-rolling the equivalent shell — for worktree isolation and for repeatability more generally (see [docs/ai-helpers/index.md](docs/ai-helpers/index.md))

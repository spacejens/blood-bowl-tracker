...
- **`packages/import-tp-live`** — server-side NestJS module that imports one TP roster, one completed TP match with its teams and competition, or one TP competition with its registered teams and trophy awards, straight through `packages/game-data`, and implements the `tpRosters.import`, `tpMatches.import` and `tpCompetitions.import` procedures `tools/import-tp` calls (see [docs/import-tp-live/index.md](docs/import-tp-live/index.md))

...
- **`packages/tp-paths`** — NestJS module with TP's URL paths as TP's own frontend requests them (a roster's, a tournament's and a match's API and page paths); pure path building with no I/O, so both `tools/download-tp` and `packages/import-tp-live` depend on it

...
- **Option ceiling.** `AskUserQuestion` allows at most 4 options per question and at most 4 questions per call. When a skill defines more than 4 options for one **multi-select** decision, split them across multiple questions in a **single** call — the harness presents them in sequence and submits all answers together, so the developer answers once, and the union of the answers is the result. Preserve the skill's listed order, set `multiSelect: true` on *every* split question (not only the first), and word each question so it reads as a continuation of one decision rather than a fresh one. Note that `header` is capped at 12 characters, so it can rarely repeat the question text. Splitting only works because the answers combine: a **single-select** decision with more than 4 mutually exclusive options cannot be split this way — narrow the list, or group the options and ask a follow-up question to drill into the chosen group.
...
Markdown files are capped at 500 lines too, enforced by the custom
`max-file-lines` markdownlint rule (in `tools/markdownlint-rules`) via the root
`.markdownlint-cli2.jsonc`. When a doc grows past the limit, the fix is the same
as this branch already did twice: split it into topic pages under the same
directory, with the original filename becoming a short index page linking to
the new ones. See `docs/import-tp/file-format.md` and
`docs/discord-bot/production-hosting.md` for the two existing examples of this
pattern.

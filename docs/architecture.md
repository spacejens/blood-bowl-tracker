...
- **`packages/import`** (shared import orchestration) — used by every `tools/import-*`; a change here reaches all importers at once. It owns:
  ...
  - **Read calls for contract procedures:** `listCompetitionGroups` and `listPositionRulesSets`, backing `competitionGroups.list` and `positionRulesSets.list`.

...
- **`packages/import`** (shared import orchestration) — used by every `tools/import-*`; a change here reaches all importers at once. It owns the shared upsert plumbing (`createUpsertImportServiceBase`) alongside the runner and batch helpers, so each entity's import service is a declarative subclass supplying only the client resource it upserts through and the wording of its per-item error message; every one of those subclasses exposes the same single method, `upsert(data, errors)`. This uses a config-object factory rather than `review-harness`'s abstract-class pattern because what varies per entity here is only *data* (which client resource, what error wording), not *behavior* — a factory taking a config object fits that better than an abstract class with template methods to override.

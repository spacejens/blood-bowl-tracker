...
packages/
  ...
  parse-tp/           — library package for reusable TP JSON-parsing logic
                        ...
                        shared with apps/discord-bot as well for a future live
                        TP import
...
- **`packages/discord-bot-usage`** (bot telemetry) — records what the bot was asked to do, not what it knows: every matched slash command, button click and select-menu selection lands in the `discord_bot_usage` schema. `packages/discord-client` calls it after each interaction has already been replied to, fire-and-forget, so a recording failure can never affect a user-facing reply. `packages/discord-client` is its only writer. `apps/discord-bot` reads it back through `InteractionEventsQueryService`, for the `/debuginteractions` and `/debugtopusers` maintainer commands; broader reporting on the captured data is still separate work
...
- **`packages/import`** (shared import orchestration) — used by every `tools/import-*`; a change here reaches all importers at once. It owns:
  ...
  - **Read calls for contract procedures:** `listCompetitionGroups` and `listPositionRulesSets`, backing `competitionGroups.list` and `positionRulesSets.list` (see `docs/api/rpc-conventions.md`).

...
packages/
  ...
  tp-paths/           — TP's roster/tournament/match/official-teams paths
                        (plus the official-teams rules-set id map) as TP's
                        frontend requests them, and the reverse: which kind
                        of TP page a URL points to; pure, no I/O, shared by
                        tools/download-tp and import-tp-live
...
- `apps/discord-bot` also imports `packages/import-tp-live` and `packages/tp-paths` directly, for the `/importtp` on-demand TP import: it classifies a TP URL with `TpPageClassifierService` and calls the matching live entry point in-process
...
- **`packages/import-tp-live`** (TP roster and match import, server-side) — imports one TP roster's team and players, one completed TP match with its teams and competition, one competition with its registered teams and trophy awards, or one rules set's official team list, directly through `packages/game-data`: the live imports (fetch via `packages/scrape-tp`, parse via `packages/parse-tp`, era auto-resolution; a live match import creates the competition it needs) and the implementations behind `tpRosters.import`, `tpMatches.import`, `tpCompetitions.import` and `tpOfficialTeams.import`, which `tools/import-tp`'s bulk run calls once per roster file, match file, competition or rules set — so a change to how a TP team, player, match, competition or official team list is imported reaches bulk and live imports together. `apps/discord-bot` calls the same four `TpLive*ImportService` entry points directly, in-process, for its `/importtp` slash command. Deliberately depends on neither `packages/import` nor `packages/api-client` (see `docs/import-tp-live/index.md`)
- **`packages/tp-paths`** (shared TP paths) — TP's roster, tournament, match and official-teams paths (plus the official-teams rules-set id map), and the reverse direction: `TpPageClassifierService` classifies a TP frontend URL as a competition, match, roster or official-teams page (or unknown) with the ids the matching `packages/import-tp-live` entry point needs, so every on-demand consumer makes the same determination. Pure and I/O-free — the frontend base URL is a parameter, not config — so the client-only `tools/download-tp` and the server-side `packages/import-tp-live` can both depend on it without either gaining the other's dependencies; `apps/discord-bot` depends on it directly too, to classify a pasted TP URL for `/importtp`
...
- **`apps/discord-bot`** (consumer) — reads imported data via `packages/game-data`; data newly landed by any importer is a candidate for a new command, fact, or insight. It is also a direct, on-demand importer in its own right: the `/importtp` slash command classifies a pasted TP URL with `packages/tp-paths`' `TpPageClassifierService` and calls the matching `packages/import-tp-live` entry point in-process

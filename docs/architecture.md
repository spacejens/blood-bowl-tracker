...
- **`packages/import`** (shared import orchestration) — used by every `tools/import-*`; a change here reaches all importers at once. It owns the shared upsert plumbing (`createUpsertImportServiceBase`) alongside the runner and batch helpers; for entities with exactly one upsert call and no other entry point, the import service is a declarative subclass supplying only the client resource it upserts through and the wording of its per-item error message, exposing a single method, `upsert(data, errors)`. Entities with more than one upsert entry point (e.g. `Competitions`, `Players`, `Matches`, `MatchEvents`) or a different runner mechanism (`ExternalSystems`, `SppAwardValues`) stay hand-written, since the base class's single `upsert` method can't express two entry points. This uses a config-object factory rather than `review-harness`'s abstract-class pattern because what varies per entity here is only *data* (which client resource, what error wording), not *behavior* — a factory taking a config object fits that better than an abstract class with template methods to override.

...
packages/
  ...
  import-tp-live/     — NestJS module importing one TP team, and its
                        players, from TP's live API (via packages/scrape-tp
                        and packages/parse-tp), with era auto-resolution; also
                        holds the team/player import services tools/import-tp
                        runs on, and TP's roster URL paths tools/download-tp
                        uses
...
- `packages/import-tp-live` imports `packages/import` like the import tools do, so a live team import also reaches the api-server over the network
...
- **`packages/import-tp-live`** (live TP import) — imports one TP team and its players straight from TP's API: fetches via `packages/scrape-tp`, parses via `packages/parse-tp`, resolves the team's era, and upserts through `packages/import`. It also holds the team/player import services `tools/import-tp`'s bulk run uses, so a change to how a TP team or player is imported reaches bulk and live imports together, and TP's roster URL paths, which `tools/download-tp` uses too. Live match and competition import build on it (see `docs/import-tp-live/index.md`)
...
- **`tools/import-tp`** (importer, TP source) — reads `tools/download-tp`'s files via `packages/parse-tp`; sibling of `tools/import-bbl`, with the same reciprocity; its team and player import run on `packages/import-tp-live`'s services
...
- **`packages/import`** (shared import orchestration) — used by every `tools/import-*`; a change here reaches all importers at once. It owns:
  ...
  - **Read calls for contract procedures:** `listCompetitionGroups`, `listPositionRulesSets` and `RacesImportService.listOngoingEras`, backing `competitionGroups.list`, `positionRulesSets.list` and `races.listOngoingEras` (see `docs/api/rpc-conventions.md`).

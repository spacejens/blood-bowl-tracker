...
- `{"splitNeeded": true, "parts": [], "unsplittableWholeBranch": {"fileCount": 140}}` — the last part, once remeasured up to the branch tip, is still over the limit and there is no further checkpoint to cut it at. This covers two cases: packing produced only one usable part (every section folded back together, or there was only one to begin with), or packing produced two or more parts that all fit fine except the last one, which self-review fix commits made after the last task's commit pushed back over the limit — in that second case the earlier, valid parts are discarded too. No single task is to blame, so no `taskNumber`/`label` is reported; `fileCount` is the unsplittable last part's own size, not `totalFileCount` — the two can differ. The caller reports this and asks the developer how to proceed, exactly as it does for `unsplittable`.

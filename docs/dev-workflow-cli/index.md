...
- `{"splitNeeded": true, "parts": [], "unsplittableWholeBranch": {"fileCount": 140}}` — packing produced only one usable part (every section folded back together, or there was only one to begin with) and it is still over the limit — typically because self-review fix commits made after the last task's commit pushed the branch tip back over the limit after `pack` had nothing left to cut. No single task is to blame, so no `taskNumber`/`label` is reported; the caller reports this and asks the developer how to proceed, exactly as it does for `unsplittable`.

...
Every issue and pull request carries at least one **kind label** — one or more of `feature`, `bug`, `development`. `write-issue` and `develop-feature` decide the kind label(s) by applying the tests below; this section is the reference they check against.

...
Reviews the whole codebase against a fixed list of criteria and files what it finds as GitHub issues (`/codebase-review`). The criteria are the `CLAUDE.md` conventions no ESLint rule enforces — service-vs-loose-function, test setup and mocking discipline, comment hygiene, dependency usage judgment — plus documentation quality and correctness under `docs/`, and a whole-picture architecture angle. One read-only subagent runs per criterion, the findings are confirmed with you before anything is created, and the result is one parent issue per review pass with a sub-issue per distinct kind of problem, grouping every occurrence of that problem rather than filing one issue per file. It changes nothing and opens no PR — the issues it files are picked up by `develop-feature` later. Run it periodically, independent of feature work; `code-hygiene` is its counterpart for the mechanical checks a tool can fix on its own.

...
- **Tool/how-to docs** — `dev-workflow-cli/`, `production-ops-cli/`, `fs-utils-cli/`, `download-tp/`, `scrape-tp/`, `import-bbl/`, `import-manual/`, `import-tp/`, `review-match/`, `review-player/`, `review-race/`, `review-star-player/`. These document how to configure, run, and operate a tool. Implementation detail — configuration schemas, CLI flags, file formats, setup and deployment steps — is expected and appropriate here, not something to omit.

...
user-install. If the database does not respond in time, a command falls back to
a themed timeout message instead of its normal reply, so it always answers

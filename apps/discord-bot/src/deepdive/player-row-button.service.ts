 ...
 * player button.
 *
 * Its own service rather than a conditional repeated at each of the four call
 * sites (the team deepdive's top players and honors, and the killer/victim
 * entries in the player deepdive's kills section), so the rule and the label
 * choice live in exactly one place. Pure and dependency-free: it performs no
 * I/O and injects nothing, matching this repo's convention that pure
 * transformation logic is still a service.
 */
@Injectable()

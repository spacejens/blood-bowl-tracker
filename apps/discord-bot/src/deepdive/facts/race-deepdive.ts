 ...
 */
export async function resolveRaceDeepdive(
  raceId: number,
  services: { races: RacesService },
): Promise<string | InteractionReplyOptions> {
  const { races } = services;

  const race: Race | undefined | null = await withDatabaseTimeout(
    races.findById(raceId),
    null,
  );
  if (race === null) {
  ...
  const topTeams: TopTeam[] | null = await withDatabaseTimeout(
    races.getTopTeamsByMatchesPlayed(raceId, MAX_LEADERBOARD_ENTRIES),
    null,
  );
  if (topTeams === null) {

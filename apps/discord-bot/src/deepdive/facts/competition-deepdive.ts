 ...
 * `withDatabaseTimeout` with a `null` sentinel so a timeout is distinguishable
 ...
 * era first.
 */
export async function resolveCompetitionDeepdive(
  competitionId: number,
  services: { competitions: CompetitionsService },
): Promise<string | InteractionReplyOptions> {
  const { competitions } = services;

  const competition: CompetitionHeader | undefined | null =
    await withDatabaseTimeout(
      competitions.findByIdWithEra(competitionId),
      null,
    );
  if (competition === null) {
  ...
  const teams: ParticipatingTeam[] | null = await withDatabaseTimeout(
    competitions.listTeams(competitionId),
    null,
  );
  if (teams === null) {

...
describe('InsightsCommandService — random pick', () => {
  ...
  it('restricts the random pick to era-supporting leaves when an era but no category is given', async () => {
    ...
    // deterministically on the last one — "stats", the final top-level key
    // in the tree, which supports era scoping. ("date" precedes it, and the
    // "eras" branch before that has no era-supporting leaves.)
